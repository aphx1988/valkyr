...
void chunkTest() {
  ...
  std::cout << "c2:baseline=" << c2->baseline << ",num=" << c2->num
            << std::endl;
  Position *pos = ChunkUtil::NewObjFrom<Position, float, float, float>(
      chunk, 100.9f, 222.22f, 333.333f);
  std::cout << "pos:x=" << pos->x << ",y=" << pos->y << ",z=" << pos->z
            << std::endl;
  std::cout << "chunk first.baseline="
            << ChunkUtil::Get<C>(sizeof(ChunkInfo), chunk)->baseline
            << std::endl;
...
void myTupleTest() {
  ...
  std::cout << "after std::tuple created,chunk head="
            << ChunkUtil::GetInfo(chunk)->head << std::endl;
  C *pc3 = &std::get<2>(*pSTuple);
  pc3->num = 666;
  std::cout << "std::get<0>(*pSTuple)=" << vget<0>(*pSTuple) << std::endl;
  std::cout << "(&std::get<2>(*pSTuple))->num=" << (&vget<2>(*pSTuple))->num
            << std::endl;
  ...
  pc4->num = 3333;
  std::cout << "vget<2>(pATuple).num = std::get<2>(*pATuple).num="
            << vget<2>(pATuple).num << std::endl;
  std::cout << "vget_ptr<C,2>(pATuple)->num=" << vget_ptr<2, C>(pATuple)->num
            << std::endl;
  auto &[x, y, c] = atuple;
...
void spanTest() {
  ...
  std::cout << "span 0: A a.i0=" << vget_ptr<0, A>(p0)->i0
            << ",C c.num=" << vget<1>(p0).num << std::endl;
  ...
  SpanUtil::ForEach<Tuple<A, C>>(span, [&](auto *pv, int i) {
    std::cout << "span" << i << ": A a.i0=" << vget_ptr<0, A>(pv)->i0
              << ",C c.num=" << vget<1>(pv).num << std::endl;
  });
  SpanUtil::Zero(span, 2);
  SpanUtil::ZeroRange(span, 6, 2);
  std::cout << "span 6: A a.i0=" << vget_ptr<0, A>(SpanUtil::Get(6, span))->i0
            << ",C c.num=" << vget<1>(SpanUtil::Get(6, span)).num << std::endl;
  std::cout << "span 7: A a.i0=" << vget_ptr<0, A>(SpanUtil::Get(7, span))->i0
            << ",C c.num=" << vget<1>(SpanUtil::Get(7, span)).num << std::endl;
  SpanUtil::ZeroAll(span);
  SpanUtil::ForEach<Tuple<A, C>>(span, [&](auto *pv, int i) {
    std::cout << "span" << i << ": " << pv
              << ",A a.i0=" << vget_ptr<0, A>(pv)->i0
              << ",C c.num=" << vget<1>(pv).num << std::endl;
  });
  ChunkAllocator::Free(chunk);
}
...
void poolTest1() {
  ...
  Chunk *chunk = ChunkAllocator::Malloc();
  Pool<Tuple<C, Rot, Position>> *pool =
      PoolUtil::CreateAndFill<MyEntt_t>(chunk);
  auto item = PoolUtil::Pop<MyEntt_t>(pool);
  vget_ptr<1, Rot>(item->data)->angle = 90.0f;
  std::cout << "item->id << item->isZero << vget_ptr<1, Rot>(item->data)->angle"
            << std::endl;
  std::cout << item->id << "," << item->isZero << ","
            << vget_ptr<1, Rot>(item->data)->angle << std::endl;
  std::cout << "=======================" << std::endl;
  int i = 0;
  PoolUtil::TraverseZero<MyEntt_t>(pool, [&i](Entt<MyEntt_t> *entt) {
    if (i % 100 == 0) {
      std::cout
          << "entt->id << entt->isZero << vget_ptr<1, Rot>(entt->data)->angle"
          << std::endl;
      std::cout << entt->id << "," << entt->isZero << ","
                << vget_ptr<1, Rot>(entt->data)->angle << std::endl;
    }
    i++;
  });
}
...
void poolTest() {
  ...
  size_t maxCapacity =
      10000000; // PoolUtil::GetMaxCapacityFor<Tuple<C,Rot,Position>>(chunk);
  ...
  for (size_t i = 0; i < maxCapacity; i++) {
    ...
    if (i % (1000000) == 0) {
      ...
      std::cout << i << " c num:" << vget_ptr<C>(it.first->data)->num
                << ",entity id=" << it.first->id
                << " isZero=" << it.first->isZero << std::endl;
    }
  }
  auto end = std::chrono::system_clock::now();
  auto duration =
      std::chrono::duration_cast<std::chrono::microseconds>(end - start);
  auto sec = double(duration.count()) * std::chrono::microseconds::period::num /
             std::chrono::microseconds::period::den;
  std::cout << "耗时:" << sec << std::endl;
  std::cout << "test count=" << maxCapacity
            << ",first pool capacity=" << pool->capacity
            << ",zero count=" << pool->zeroCount << std::endl;
  ...
  for (size_t i = 0; i < maxCapacity; i++) {
    ...
    if (i % (1000000) == 0) {
      ...
      // std::cout << i << "vec[i].baseline:" << std::get<0>(vec[i]).baseline <<
      // std::endl;
    }
  }
  ...
  auto sec2 = double(duration2.count()) *
...
void poolGroupTest() {
  ...
  for (int i = 0; i < num; i++) {
    ...
    if (i % 10 == 0) {
      ...
      std::cout << "r.second->frontZero->id="
                << (!r.second->frontZero ? "null"
                                         : "" + r.second->frontZero->id)
                << std::endl;
...
struct CodeTeam {
  ...
  CodeTeam(TaskQueue &queue)
      : codeRepo(queue), running(true) //,workersTouchingFish(queue.len,true)
  {}
};
...
public:
  ...
  void exec() { cout << std::this_thread::get_id() << ":" << m_v << std::endl; }
};
...
TaskSeq genTaskSeq() {
  ...
  seq.push(group1);
  vptr<Task> t20 =
      vmake_ptr<ComputeTask>(std::initializer_list<int>{1, 2, 3, 4, 5, 6});
  vptr<Task> t21 =
      vmake_ptr<ComputeTask>(std::initializer_list<int>{1, 20, 300});
